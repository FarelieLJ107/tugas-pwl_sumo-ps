...
export const RentalPanel: React.FC<RentalPanelProps> = ({ token }) => {
  ...
  const handleRentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/rental-bawa-pulang/${selectedRental.id_rental}/rent`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleReturnSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/rental-bawa-pulang/${returningRental.id_rental}/return`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleToggleMaintenance = async (rental: RentalKonsol) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const res = await fetch(`/api/rental-bawa-pulang/${rental.id_rental}/${endpoint}`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (res.ok) {
